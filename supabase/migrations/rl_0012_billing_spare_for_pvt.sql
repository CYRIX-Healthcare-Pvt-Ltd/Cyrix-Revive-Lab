-- =====================================================================
-- rl_0012 — Billing spare, asked for Pvt
-- =====================================================================
--
-- A spare from a private customer's equipment may be billed to them, and
-- the Revive Lab needs to know that before it starts. So when the BEMMP is
-- Pvt, the route card asks one more thing: Billing spare, Yes or No, No
-- unless changed.
--
-- Which BEMMP asks is a flag on the BEMMP, set for Pvt here, rather than
-- a comparison with the text "Pvt": an admin renaming it to "Private"
-- must not quietly stop the question being asked. Everywhere else the
-- answer is null — not No, because nobody was asked.
--
-- The raise function takes the answer as a last, optional argument, so
-- the app as it is live keeps raising tickets; under Pvt, a call without
-- it records No, the form's own default.

alter table public.revive_bemmp_projects
  add column if not exists asks_billing boolean not null default false;

comment on column public.revive_bemmp_projects.asks_billing is
  'The route card asks "Billing spare" (Yes/No, default No) for a ticket under this BEMMP. Set for Pvt.';

update public.revive_bemmp_projects
   set asks_billing = true
 where lower(btrim(code)) = 'pvt';

alter table public.revive_tickets
  add column if not exists billing_spare boolean;

comment on column public.revive_tickets.billing_spare is
  'Whether the spare is billed to the customer. Asked only under a BEMMP that asks_billing (Pvt); null where it was not asked.';

-- ---------------------------------------------------------------------
-- Raising a ticket: the answer, as a last optional argument
-- ---------------------------------------------------------------------
...
create function public.revive_raise_ticket(
  ...
  p_billing_spare     boolean default null)
returns jsonb
...
begin
  ...
  select case when bp.asks_billing then coalesce(p_billing_spare, false) end
    into billing
    from revive_bemmp_projects bp
   where bp.id = p_bemmp_id and bp.is_active;
  if not found then
  ...
  */
  ...
  insert into revive_tickets (
    trc_kind, trc_id, facility, state, bemmp_id, billing_spare, district, source_ticket_no,
...
-- ---------------------------------------------------------------------
-- The list carries the answer. A changed return type is a drop and create.
-- ---------------------------------------------------------------------
